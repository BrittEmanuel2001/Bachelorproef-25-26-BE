...
Wat voor een lange tijd voornamelijk geassocieerd werd met ervaren werknemers in veeleisende functies, blijkt vandaag echter een fenomeen dat zich niet beperkt tot één specifieke leeftijdscategorie. Een burn-out kan in principe zich op elke leeftijd manifesteren. Toch wijzen recente cijfers erop dat vooral jongvolwassenen steeds vaker getroffen worden \autocite{RIZIV2023_invaliditeit_burnout}. Deze verhoogde kwetsbaarheid kan zowel worden verklaard door individuele factoren als door maatschappelijke invloeden. Enerzijds spelen persoonlijke kenmerken zoals perfectionisme, stressgevoeligheid en moeilijkheden met het stellen van grenzen een belangrijke rol. Anderzijds bevinden jongvolwassenen zich in een levensfase die gekenmerkt wordt door ingrijpende identiteits- en toekomstvragen, waarbij zij geconfronteerd worden met maatschappelijke tendensen zoals toenemende prestatiedruk, permanente digitale connectiviteit en sociale vergelijking. Deze combinatie van interne en externe factoren maakt dat zij bijzonder vatbaar zijn voor chronische stress en burn-outklachten. Hierdoor is mentale gezondheid uitgegroeid tot één van de meest urgente maatschappelijke uitdagingen voor deze generatie.

\newpage

Om dit probleem onder handen te nemen hebben verscheidene software- en productontwikkelaars een poging gewaagd tot het ontwikkelen van toepassingen die beogen stress en burn-out te verminderen, wat heeft geleid tot een explosieve groei van mentale welzijn-apps. 

\vspace{1em}

De aanhoudende stijging van burn-outgevallen suggereren echter dat deze apps tot op heden slechts een beperkter effect hebben. Dit roept de vraag op hoe dergelijke toepassingen kunnen worden verbeterd en succesvol kunnen worden ingezet zodat ze jongvolwassenen effectiever ondersteunen bij het omgaan met de dagelijkse druk en de onzekerheden die onlosmakelijk verbonden zijn aan hun levensfase.
...
\begin{itemize}
    ...
    \item Hoe beïnvloeden persoonlijke en externe factoren \textit{(werk-, studie- en sociale druk)} de gebruikerservaring van digitale toepassingen?
    \item Waar falen bestaande apps in het ondersteunen van mentale gezondheid en het verminderen van stress of burn-out?
    \item Welke functionaliteiten en ontwerpprincipes worden momenteel gebruikt in apps voor mentaal welzijn?
\end{itemize}
...
In Hoofdstuk~\ref{ch:requirementsanalyse} worden de functionele en niet-functionele requirements gedefinieerd op basis van de eerdere literatuurstudie en een expertinterview. Daarnaast worden ontwerpprincipes geformuleerd die de vertaalslag maken van deze inzichten en requirements naar concrete richtlijnen voor stressreducerend ontwerp in applicaties.

\vspace{1em}

In Hoofdstuk~\ref{ch:shortlist} wordt de eerder opgestelde longlist van mentale gezondheidsapps herleid tot een shortlist van relevante toepassingen. Deze selectie gebeurt op basis van de \acrlong{fr}s, waarna de geselecteerde apps verder worden geëvalueerd aan de hand van de ontwerpprincipes. Op deze manier worden zowel relevante functionaliteiten als sterke en minder effectieve ontwerpkeuzes geïdentificeerd, die als input dienen voor de verdere uitwerking van de \acrshort{poc}, waarin deze ontwerpprincipes concreet worden toegepast en getoetst.

\vspace{1em}

In Hoofdstuk~\ref{ch:poc} wordt een \acrlong{poc} uitgewerkt om aan te tonen hoe de opgestelde ontwerpprincipes kunnen worden toegepast bij de ontwikkeling van een applicatie ter ondersteuning van het voorkomen, herkennen en aanpakken van chronische overspanning. Hiervoor worden de eerder opgestelde functionele en niet-functionele requirements, samen met de ontwerpprincipes, als uitgangspunt genomen. Na de selectie van een geschikte technologie worden mock-ups uitgewerkt, waarna de \acrshort{poc} wordt geïmplementeerd in de eerder gekozen technologie.

\vspace{1em}

In Hoofdstuk~\ref{ch:evaluatie} wordt de uitgewerkte \acrshort{poc} geëvalueerd aan de hand van een heuristische analyse. Hierbij wordt de implementatie getoetst aan de eerder opgestelde requirements en ontwerpprincipes. De resultaten van deze evaluatie geven inzicht in de kwaliteit van het prototype en mogelijke verbeterpunten voor verdere ontwikkeling.
