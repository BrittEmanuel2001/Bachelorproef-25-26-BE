...
function groupByMonth(entries: ReflectionData[]): Record<string, ReflectionData[]> {
...
export function HistoryContent() {
    ...
    const [selectedEntry, setSelectedEntry] = useState<ReflectionData | null>(null);
        ...
        <>
            <View style={styles.toolbar}>
                <Pressable style={styles.iconButton}>
                    <IconSymbol size={20} name="search" color={colors.darkBlue}/>
                </Pressable>
                <Pressable
                    onPress={() => setViewMode("list")}
                    style={[styles.iconButton, viewMode === "list" && styles.selectedIconButton]}
                >
                    <IconSymbol size={20} name="list" color={viewMode === "list" ? colors.white : colors.darkBlue}/>
                </Pressable>

                <Pressable
                    disabled
                    onPress={() => setViewMode("calendar")}
                    style={[styles.iconButton, viewMode === "calendar" && styles.selectedIconButton]}
                >
                    <IconSymbol size={20} name="calendar" color={viewMode === "calendar" ? colors.white : colors.darkBlue}/>
                </Pressable>
            </View>
            ...
            >
                {sortedMonths.map((monthKey, index) => (
                    <View key={monthKey} style={[styles.monthSection, index === sortedMonths.length - 1 && {marginBottom: 0} ]}>
                                            ...
                                            <View style={styles.entryCard}>
                                                <View style={styles.cardContent}>
                                                    {(() => {
                                                        ...
                                                        return moodOpt ? (
                                                            <View style={[styles.moodChip, { backgroundColor: moodOpt.color }]}>
                                                                <IconSymbol size={16} name={moodOpt.icon!} color={colors.white} />
                                                                <Text style={styles.moodChipText}>{moodOpt.label}</Text>
                                                            </View>
                                                        ) : null;
                                                    })()}
                                                    
                                                    <Text style={styles.stressPreview} numberOfLines={2}>{entry.balanceNote || "-"}</Text>
                                                </View>

                                                <IconSymbol size={20} name="chevron.right" color={colors.darkBlue} />
                                            </View>
                                        </Pressable>
                                    );
                                })}
